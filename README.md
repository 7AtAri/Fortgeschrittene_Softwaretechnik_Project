...
- being free of side effects is not possible, since the project relies on user input and the interaction of the bot with the website. In fact it relies on side effects, because it manipulates the browser object in each of the bots actions. Also it all finally had to be wrapped in a Scheduler function, so the Bot try can do its job more than one time, if the appointment booking was not successful in the first place. Still, I tried to keep my functions as side-effect free as possible. 
